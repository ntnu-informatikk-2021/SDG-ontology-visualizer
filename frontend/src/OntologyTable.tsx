    ...
    <tr>
      <td>
        {relatedOntology.Subject ? (
          <a onClick={() => onClick(relatedOntology.Subject)}>
      ...
      <td>
        {relatedOntology.Object ? (
          <a onClick={() => onClick(relatedOntology.Object)}>
            {relatedOntology.Object.name}
          </a>
        ) : (
          <span>{selectedNode.name}</span>
        )}
...
const OntologyTable: React.FC = () => {
  ...
  return (
    <table>
      ...
      {ontologies &&
        ontologies.map((ont) => renderOntology(selectedNode, ont, clickNode))}
    </table>
  );
};
